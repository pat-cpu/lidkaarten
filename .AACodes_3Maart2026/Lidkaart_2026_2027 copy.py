...
# Posities / offsets (bijgeschaafd voor 85x54)
HEADER_OFFSET_Y = 4 * MM
PRICE_OFFSET_Y = 2 * MM
NAME_OFFSET_X = -4
NAME_OFFSET_Y = 7 * MM
QR_OFFSET_X = 6
...
CONTACT_1 = "Gsm Patrick: 0486/26.23.42"
CONTACT_2 = "Walter: 0475/24.05.15"

# ------------------------------------------------------------
# BACK instellingen
# ------------------------------------------------------------
BACK_WEEKDAY_NAME = "MAANDAG"
BACK_DEFAULT_TIME = "13u30"
BACK_JANUARY_TIME = "13u00"

# Back layout (iets kleiner)
BACK_HEADER_FONT = 13
BACK_SUB_FONT = 10
BACK_DATES_FONT = 10
BACK_NOTE_FONT = 9

BACK_HEADER_OFFSET_Y = 6
BACK_BLOCK_GAP = 12
BACK_LINE_GAP = 10
BACK_NOTE_GAP = 12

# Duplex fine-tuning shift (gebruikt door single + duplex)
BACK_IMPOSE_X_SHIFT = 2.3 * MM

# Kaart-impositie (A4 raster)
A4_W, A4_H = 595, 842  # portrait (points)
GRID_COLS, GRID_ROWS = 2, 5
PAGE_MARGIN = 18       # points (~6.35 mm)
CELL_MARGIN = 4        # points (~1.41 mm) -> iets groter kaartje in cel
...
def card_svg(code13: str, full_name: str, qr_payload=None):
    ...
    # ✅ ronding kleiner voor creditcard
    ...
    # Header / prijs kleiner
    parts.append(
        f'<text x="{PADDING}" y="{header_y}" font-family="{FONT}" font-size="12" font-weight="bold">{HEADER_TEXT}</text>'
    )
    parts.append(
        f'<text x="{PADDING}" y="{price_y}" font-family="{FONT}" font-size="14" font-weight="bold">{PRICE_TEXT}</text>'
    )

    # Barcode rechtsboven (iets lager)
    ...
    # Contact onderaan rechts (kleiner)
    parts.append(
...
# ------------------------------------------------------------
# Belgische feestdagen (incl. Pasen-afhankelijk)
# ------------------------------------------------------------
...
def belgian_public_holidays(year: int) -> set[date]:
    ...
    ascension = easter + timedelta(days=39)        # donderdag
    whit_monday = easter + timedelta(days=50)      # maandag
    ...
    return {
        ...
        date(year, 8, 15),   # O.L.V. Hemelvaart
...
# ------------------------------------------------------------
# AUTO-GENERATED SEASON DATES (fallback)
# Regel:
# - 4de maandag van de maand
# - december: 3de maandag
# - indien 4de maandag feestdag is → 3de maandag
# ------------------------------------------------------------
BACK_N_FOR_MONTHS = {
    9: 4, 10: 4, 11: 4, 12: 3,
...
def compute_season_dates(year_start: int) -> list[date]:
    ...
    # Sep-Dec in startjaar
    for m in (9, 10, 11, 12):
        n = BACK_N_FOR_MONTHS[m]
        d = nth_weekday_of_month(year_start, m, 0, n)  # maandag = 0
        if n == 4 and is_holiday_be(d):
            d = nth_weekday_of_month(year_start, m, 0, 3)
        out.append(d)

    # Jan-Jun in volgend jaar
...
def back_card_svg(season_start: int, manual_dates=None):
    ...
    logo_pack = logo_image_fragment()
    logo_h = logo_pack[2] if logo_pack else 0

    cx = CARD_W / 2
    header_y = PADDING + BACK_HEADER_OFFSET_Y + logo_h

    sub_y = header_y + BACK_BLOCK_GAP
    l1_y = sub_y + BACK_BLOCK_GAP
    l2_y = l1_y + BACK_LINE_GAP
    l3_y = l2_y + BACK_LINE_GAP
    note_y = l3_y + BACK_NOTE_GAP

    parts = [f'<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="{CARD_W}" height="{CARD_H}">']
    parts.append('<rect width="100%" height="100%" rx="6" ry="6" fill="white"/>')
